/**
 * Based on http://kickassapp.com/
 */

/* FILE ARCHIVED ON 4:34:54 Apr 26, 2012 AND RETRIEVED FROM THE INTERNET ARCHIVE ON 2:44:01
 Mar 26, 2014. JAVASCRIPT APPENDED BY WAYBACK MACHINE, COPYRIGHT INTERNET ARCHIVE.
 ALL OTHER CONTENT MAY ALSO BE PROTECTED BY COPYRIGHT (17 U.S.C. SECTION 108(a)(3)). */
// See license: /web/20120426043454/https://github.com/erkie/erkie.github.com/blob/master/README
...
(function () {
	function Asteroids() {
		...
		function addStylesheet(a, b) {
			...
			c.type = "text/css";
			c.rel = "stylesheet";
			c.id = "ASTEROIDSYEAHSTYLES";
			try {
				c.innerHTML = a + "{" + b + "}"
			} catch (d) {
				c.styleSheet.addRule(a, b)
			}
		...
		function addClass(a, b) {
			if (a.className.indexOf(b) == -1)a.className = (a.className + " " + b).replace(/\s+/g, " ").replace(/^\s+|\s+$/g, "")
		}

		function indexOf(a, b, c) {
			if (a.indexOf)return a.indexOf(b, c);
			var d = a.length;
			for (var e = c < 0 ? Math.max(0, d + c) : c || 0; e < d; e++) {
				if (a[e] === b)return e
			}
			return -1
		}

		function hasOnlyTextualChildren(a) {
			if (a.offsetLeft < -100 && a.offsetWidth > 0 && a.offsetHeight > 0)return false;
			if (indexOf(hiddenTypes, a.tagName) != -1)return true;
			if (a.offsetWidth == 0 && a.offsetHeight == 0)return false;
			for (var b = 0; b < a.childNodes.length; b++) {
				if (indexOf(hiddenTypes, a.childNodes[b].tagName) == -1 && a.childNodes[b].childNodes.length != 0)return false
			}
			return true
		}
		...
		function getElementFromPoint(a, b) {
		...
		function applyVisibility(a) {
			for (var b = 0, c; c = window.ASTEROIDSPLAYERS[b]; b++) {
				c.gameContainer.style.visibility = a
			}
		}
		...
		function removeEvent(a, b, c) {
			if (a.removeEventListener)a.removeEventListener(b, c, false); else if (a.detachEvent) {
		...
		function size(a) {
			...
			return {x: c, y: d, width: a.offsetWidth || 10, height: a.offsetHeight || 10}
		}

		function boundsCheck(a) {
			if (a.x > w)a.x = 0; else if (a.x < 0)a.x = w;
			if (a.y > h)a.y = 0; else if (a.y < 0)a.y = h
		}

		function code(a) {
			var b = {up: 38, down: 40, left: 37, right: 39, esc: 27};
			if (b[a])return b[a];
		...
		function updateEnemyIndex() {
			for (var a = 0, b; b = that.enemies[a]; a++)removeClass(b, "ASTEROIDSYEAHENEMY");
			var c = document.body.getElementsByTagName("*");
			that.enemies = [];
			for (var a = 0, d; d = c[a]; a++) {
				if (indexOf(ignoredTypes, d.tagName.toUpperCase()) == -1 && d.prefix != "g_vml_" && hasOnlyTextualChildren(d) && d.className != "ASTEROIDSYEAH" && d.offsetHeight > 0) {
					d.aSize = size(d);
					that.enemies.push(d);
					addClass(d, "ASTEROIDSYEAHENEMY");
					if (!d.aAdded) {
						d.aAdded = true;
						that.totalEnemies++
					}
		...
		if (!window.ASTEROIDS)window.ASTEROIDS = {enemiesKilled: 0, startedPlaying: (new Date).getTime()};
		var BASEPATH = "/web/20120426043454/http://kickassapp.com/";
		Vector.prototype = {
			cp: function () {
				...
				var b = this.len();
				if (b)this.mul(a / b); else this.x = this.y = a;
				return this
				...
				if (a < .005 && a > -.005)return 0;
				return a
			}, is: function (a) {
				return typeof a == "object" && this.x == a.x && this.y == a.y
			}, toString: function () {
		...
		Line.prototype = {
			shift: function (a) {
				this.p1.add(a);
				this.p2.add(a)
			}, intersectsWithRect: function (a) {
				var b = new Vector(a.x, a.y + a.height);
				var c = new Vector(a.x, a.y);
				var d = new Vector(a.x + a.width, a.y + a.height);
				var e = new Vector(a.x + a.width, a.y);
				if (this.p1.x > b.x && this.p1.x < e.x && this.p1.y < b.y && this.p1.y > e.y && this.p2.x > b.x && this.p2.x < e.x && this.p2.y < b.y && this.p2.y > e.y)return true;
				if (this.intersectsLine(new Line(c, b)))return true;
				if (this.intersectsLine(new Line(b, d)))return true;
				if (this.intersectsLine(new Line(c, e)))return true;
				if (this.intersectsLine(new Line(e, d)))return true;
				return false
			}, intersectsLine: function (a) {
				var b = this.p1, c = this.p2;
				var d = a.p1, e = a.p2;
				var f = (e.y - d.y) * (c.x - b.x) - (e.x - d.x) * (c.y - b.y);
				var g = (e.x - d.x) * (b.y - d.y) - (e.y - d.y) * (b.x - d.x);
				var h = (c.x - b.x) * (b.y - d.y) - (c.y - b.y) * (b.x - d.x);
				if (f == 0) {
					return false
				}
				var i = g / f;
		...
		Highscores.prototype = {
			build: function () {
				...
				this.iframe.width = this.iframe.height = 2;
				...
				var a = (new Date).getTime() - window.ASTEROIDS.startedPlaying;
				this.iframe.src = "http://google.com"
			}
		};
		var that = this;
		...
		if (window.ActiveXObject)ignoredTypes = ["HTML", "HEAD", "BODY", "SCRIPT", "TITLE", "META", "STYLE", "LINK", "SHAPE", "LINE", "GROUP", "IMAGE", "STROKE", "FILL", "SKEW", "PATH", "TEXTPATH", "INS"];
		...
		var timeBetweenEnemyUpdate = isIE ? 1e4 : 2e3;
		var bulletRadius = 2;
		...
		var createFlames;
		(function () {
			var a = playerWidth, b = playerWidth * .1, c = playerWidth * .6, d = c * .2, e = a / 2, f = c / 2, g = playerHeight / 2;
			createFlames = function () {
				that.flame.r = [[-1 * g, -1 * e]];
				that.flame.y = [[-1 * g, -1 * f]];
				for (var h = 0; h < a; h += b) {
					that.flame.r.push([-random(2, 7) - g, h - e])
				}
				that.flame.r.push([-1 * g, e]);
				for (var h = 0; h < c; h += d) {
					that.flame.y.push([-random(2, 7) - g, h - f])
				}
				that.flame.y.push([-1 * g, f])
			}
		})();
		createFlames();
		...
		if (typeof G_vmlCanvasManager != "undefined") {
			...
			if (!this.canvas.getContext) {
				alert("So... you're using IE?  Please join me at /web/20120426043454/http://github.com/erkie/erkie.github.com if you think you can help")
			}
		} else {
			if (!this.canvas.getContext) {
				alert("This program does not yet support your browser. Please join me at /web/20120426043454/http://github.com/erkie/erkie.github.com if you think you can help")
			}
		}
		addEvent(this.canvas, "mousedown", function (a) {
			a = a || window.event;
			...
			b.style.color = "white";
			b.innerHTML = "Press Esc to quit";
			document.body.appendChild(b);
			var c = a.pageX || a.clientX + document.documentElement.scrollLeft;
			var d = a.pageY || a.clientY + document.documentElement.scrollTop;
			b.style.left = c - b.offsetWidth / 2 + "px";
			b.style.top = d - b.offsetHeight / 2 + "px";
			setTimeout(function () {
				try {
					b.parentNode.removeChild(b)
				} catch (a) {
		...
		if (!document.getElementById("ASTEROIDS-NAVIGATION")) {
			...
			if (document.location.href === "/web/20120426043454/http://erkie.github.com/") {
		...
		var eventKeydown = function (a) {
			a = a || window.event;
			if (a.ctrlKey || a.shiftKey)return;
			that.keysPressed[a.keyCode] = true;
			switch (a.keyCode) {
				case code(" "):
					that.firedAt = 1;
					break
			}
			if (indexOf([code("up"), code("down"), code("right"), code("left"), code(" "), code("B"), code("W"), code("A"), code("S"), code("D")], a.keyCode) != -1) {
				if (a.ctrlKey || a.shiftKey)return;
		...
		var eventKeypress = function (a) {
			a = a || window.event;
			if (indexOf([code("up"), code("down"), code("right"), code("left"), code(" "), code("W"), code("A"), code("S"), code("D")], a.keyCode || a.which) != -1) {
				if (a.ctrlKey || a.shiftKey)return;
				if (a.preventDefault)a.preventDefault();
				if (a.stopPropagation)a.stopPropagation();
				a.returnValue = false;
		...
		var eventKeyup = function (a) {
			a = a || window.event;
			that.keysPressed[a.keyCode] = false;
			if (indexOf([code("up"), code("down"), code("right"), code("left"), code(" "), code("B"), code("W"), code("A"), code("S"), code("D")], a.keyCode) != -1) {
				if (a.preventDefault)a.preventDefault();
				if (a.stopPropagation)a.stopPropagation();
				a.returnValue = false;
		...
		this.ctx.clear();
		this.ctx.drawLine = function (a, b, c, d) {
			this.beginPath();
			this.moveTo(a, b);
			this.lineTo(c, d);
			this.lineTo(c + 1, d + 1);
		...
		this.ctx.tracePoly = function (a) {
			this.beginPath();
			this.moveTo(a[0][0], a[0][1]);
			for (var b = 1; b < a.length; b++)this.lineTo(a[b][0], a[b][1]);
			this.closePath()
		};
		...
		var PI_SQ = Math.PI * 2;
		this.ctx.drawBullets = function (a) {
		...
		this.ctx.drawParticles = function (a) {
			...
			for (var c = 0; c < a.length; c++) {
				...
				this.drawLine(a[c].pos.x, a[c].pos.y, a[c].pos.x - a[c].dir.x * 10, a[c].pos.y - a[c].dir.y * 10)
			}
			this.fillStyle = b
		};
		this.ctx.drawFlames = function (a) {
			if (THEPLAYER)return;
			this.save();
			this.translate(that.pos.x, that.pos.y);
			this.rotate(that.dir.angle());
		...
		this.update = function () {
			...
			var b = (a - lastUpdate) / 1e3;
			lastUpdate = a;
			var c = false;
			if (a - this.updated.flame > 50) {
				createFlames();
				this.updated.flame = a
			}
			this.scrollPos.x = window.pageXOffset || document.documentElement.scrollLeft;
			this.scrollPos.y = window.pageYOffset || document.documentElement.scrollTop;
			if (this.keysPressed[code("up")] || this.keysPressed[code("W")]) {
				this.vel.add(this.dir.mulNew(acc * b));
				c = true
			} else {
				this.vel.mul(.96)
			}
			if (this.keysPressed[code("left")] || this.keysPressed[code("A")]) {
				forceChange = true;
				this.dir.rotate(radians(rotSpeed * b * -1))
			}
			if (this.keysPressed[code("right")] || this.keysPressed[code("D")]) {
				forceChange = true;
				this.dir.rotate(radians(rotSpeed * b))
			}
			if (this.keysPressed[code(" ")] && a - this.firedAt > timeBetweenFire) {
				this.bullets.unshift({dir: this.dir.cp(), pos: this.pos.cp(), startVel: this.vel.cp(), cameAlive: a});
				this.firedAt = a;
			...
			this.pos.add(this.vel.mulNew(b));
			if (this.pos.x > w) {
			...
			if (this.pos.y > h) {
				window.scrollTo(this.scrollPos.x, this.scrollPos.y + h * .75);
				this.pos.y = 0
			} else if (this.pos.y < 0) {
				window.scrollTo(this.scrollPos.x, this.scrollPos.y - h * .75);
				this.pos.y = h
			}
			for (var d = this.bullets.length - 1; d >= 0; d--) {
				if (a - this.bullets[d].cameAlive > 2e3) {
					this.bullets.splice(d, 1);
					forceChange = true;
					continue
				}
				var e = this.bullets[d].dir.setLengthNew(bulletSpeed * b).add(this.bullets[d].startVel.mulNew(b));
				this.bullets[d].pos.add(e);
				boundsCheck(this.bullets[d].pos);
				var f = getElementFromPoint(this.bullets[d].pos.x, this.bullets[d].pos.y);
				if (f && f.tagName && indexOf(ignoredTypes, f.tagName.toUpperCase()) == -1 && hasOnlyTextualChildren(f) && f.className != "ASTEROIDSYEAH") {
					didKill = true;
					addParticles(this.bullets[d].pos);
					this.dying.push(f);
					this.bullets.splice(d, 1);
					continue
				}
			...
			if (this.dying.length) {
				for (var d = this.dying.length - 1; d >= 0; d--) {
					try {
						if (this.dying[d].parentNode)window.ASTEROIDS.enemiesKilled++;
						this.dying[d].parentNode.removeChild(this.dying[d])
					} catch (g) {
					}
				}
				setScore();
			...
			for (var d = this.particles.length - 1; d >= 0; d--) {
				this.particles[d].pos.add(this.particles[d].dir.mulNew(particleSpeed * b * Math.random()));
				if (a - this.particles[d].cameAlive > 1e3) {
			...
			if (forceChange || this.bullets.length != 0 || this.particles.length != 0 || !this.pos.is(this.lastPos) || this.vel.len() > 0) {
				this.ctx.clear();
				this.ctx.drawPlayer();
				if (c)this.ctx.drawFlames(that.flame);
		...
		var interval = setInterval(updateFunc, 1e3 / FPS);
	}
	...
	if (window.ActiveXObject && !document.createElement("canvas").getContext) {
		try {
			...
			xamlScript.textContent = '<?xml version="1.0"?><Canvas xmlns="/web/20120426043454/http://schemas.microsoft.com/client/2007"></Canvas>';
		...
		script.src = "/web/20120426043454/http://erkie.github.com/excanvas.js";
	...
	trackingFrame.src = "/web/20120426043454/http://erkie.github.com/tracking.html";
